...
class FlyerAgentEnv(object):
    ...
    def __init__(self):
        self.state = [0.,0.]
        self.action = np.zeros(4).astype(float)
...
#         displacement = np.sqrt((self.state[0]-goal_x)**2 + (self.state[1]-goal_y)**2)
#         reward = max_displacement - displacement
        
        return reward
      
    def calc_next_state(self):
        s_prime = [0]*2 # python's empty list creation (ugh)
        DX = 1 # ie delta x
        DY = 2
...
#                             [0,0,0,1,    0, -DA,  DA,   0, -DA,  DA,   0, -DA,  DA,  0]])
        transition_matrix = np.array([[1,0,    0, -DX,   0,  DX,  0],
                                      [0,1,    0,  DY,2*DY,  DY,  G]])
